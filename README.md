...
- ![|800](README/image-20221026141608867.png)
- 栈由高地址向低地址生长
...
- 高版本系统中每个进程独占一个线性地址空间, 因此不再需要LDT, 只需要使用GDT. 同时一个页目录表
- 保护模式: 
	- 16位变32位
	- 寻址方式: 分段->线性地址->分页
	- 打开保护模式后除了CR3, 页目录表, 页表中存有物理地址, 其他地方使用的都是线性地址, 从线性地址到物理地址的转换是由MMU根据页目录表, 页表, 页的设置由硬件自动完成的.
	- 内存分段机制是为了让程序能够链接子程序 (函数), 而内存分页机制是为了==隔离不同用户的内存空间/虚拟内存== (IA32 3.6分页概述)
- 常用数值:
	- 1KB: 0X400
	- 4KB: 0X1000
...
硬盘读写缓冲的存在是为了快, 之所以能快是因为可能有多个进程对硬盘读写, 映射到内存的缓冲区能被复用. 如果没有被复用, 反而会更慢些. 因此缓冲区设计的核心是==使已经进入缓冲区的数据停留尽可能久==, 这样才能最大程度复用.
